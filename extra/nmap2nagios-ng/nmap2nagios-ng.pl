...
#Copyright (c) 2008-2009 Michel Sigloch
...
$options_ref->{'configuration_file'} = $data_ref->{'Path'} . '/nmap2nagios-ng_2x.conf' if (!$options_ref->{'configuration_file'});
...
{
print("\n\tYou have not choosen a Nagios-Version! Using default (3.x) \n");
$options_ref->{'configuration_file'} = $data_ref->{'Path'} . '/nmap2nagios-ng_3x.conf' if (!$options_ref->{'configuration_file'});
$options_ref->{'configuration_file'} = './' . $options_ref->{'configuration_file'} if ($options_ref->{'configuration_file'} !~ /^(\/|\.\/)/);
}
